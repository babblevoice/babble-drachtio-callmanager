...
*/
  constructor( req, res ) {
    ...
    */
    ...
    callstore.set( this )
    callmanager.options.em.emit( "call", this )
  }
  ...
  */
  async _onlatebridge() {
    ...
    if( this.parent.established ) {
      ...
      let dlg = await this._dialog.ack( localsdp.toString() )
      ...
      this.parent.answer( { "preferedcodecs": remotecodecs } )
        ...
        .catch( () => {
          if( false !== this._promises.reject.newuac ) this._promises.reject.newuac( this )
          if( false !== this._timers.newuac ) clearTimeout( this._timers.newuac )
  ...
  */
  _onauth( req, res ) {
    ...
    if( false !== this._promises.resolve.auth ) {
      ...
      digestauth( {
        ...
        this._em.emit( "presence.dialog.out", p )
        callmanager.options.em.emit( "presence.dialog.out", p )
  ...
  */
  _cleanup() {
    ...
    if( false !== this._promises.reject.auth ) {
      this._promises.reject.auth( this )
    }
    ...
    if( false !== this._promises.resolve.events ) {
      this._promises.resolve.events( this )
      this._promises.resolve.events = false
    }

    if( false !== this._promises.resolve.hangup ) {
      this._promises.resolve.hangup( this )
      this._promises.resolve.hangup = false
    }
