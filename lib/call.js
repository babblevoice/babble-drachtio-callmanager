...
class call {
  ...
  */
  ...
  async #answerparent() {
    ...
    if( this.parent.callbacks && this.parent.callbacks.onconnect ) await this.parent.callbacks.onconnect()
  ...
  */
  ...
  get unobtainablesoup() {
    if( this._unobtainablesoup ) {
      return this._unobtainablesoup
    }
  ...
  */
  ...
  /**
    ...
    @private
  */
  async reinvite( newchannel ) {

    if ( newchannel )
      this.channels.audio = newchannel

    /* make sure our SDP is up to date */
    this.#configurelocalsdp()

    const requestoptions = {}
    requestoptions.method = "INVITE"
    if( this.sdp.local ) {
    ...
    if( remoteresponse?.msg?.body ) {
      this.sdp.remote = sdpgen.create( remoteresponse.msg.body )
      this.sdp.remote.setdynamepayloadtypes( this.sdp.local )
    }
  }
   ...
   */
  async mix( othercall, sendreinvite=true ) {
    if( !othercall.channels.audio ) return
    if( !this.channels.audio ) return

    await this.#mixdifferentnode( othercall, sendreinvite )
   ...
   * @returns { Promise }
   */
  async #mixdifferentnode( othercall, sendreinvite ) {
    ...
    /* this will be based on the remote SDP from the origional - TODO we need to test if the new remote SDP changes */
    ...
    othercall.channels.audio = newchannel

    othercall.bond( this )
    if( sendreinvite ) await othercall.reinvite()
  ...
  */
  ...
  async _runattendedxfer( req, res, replaces, replacesuri ) {
    ...
    await Promise.all(
      callswithaudio.map( ( call ) =>
        call.waitforanyevent( { action: "mix", event: "finished" }, 0.5 )
      )
    )
    ...
    */
    try {
      await b_2._notifyreferstart()
    } catch( e ) {
      console.trace( e )
      b_2._notifyreferfail()
      return
    }
  ...
  #parsesrtpsetup(sdp) {
    ...
    if( !secure.roleset ) {
      if( "active" === audio.setup ) {
        ...
        // remote offers actpass; prefer passive so the remote (browser)
        // initiates DTLS after STUN - avoids sending ClientHello to
        // an unconfirmed address in projectrtp
        secure.dtlscontroller = false
      }
      secure.roleset = true
    }
  ...
  #getsrtpsetupforchannel( media ) {
    ...
    if( "actpass" == media.setup ) return "passive"
    return "active"
  }
  ...
  */
  ...
  async hangup( reason, tone = false, source = "us" ) {
    try {
      ...
      if( this.state.established ) {
        ...
        await this.#hangupnotestablish( reason )
   ...
   * @returns { Promise }
   */
  async #hangupnotestablish( reason ) {
    try {
      this.canceled = true
      if( this._req ) {
        if (
  ...
  */
   ...
   * @param { object } [ channeldef ]
   */
  async #openchannelsfornewuac( channeldef ) {
  ...
  #configureto() {
    ...
    if( this.options?.calledid?.number ) {
