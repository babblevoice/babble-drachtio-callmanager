...
*/
  ...
  */
  ...
  @param { string } [ options.contact ] - The contact string, this will be used if no entity is supplied
  ...
  @param { entity } [ options.entity ] - if this is a local user the entity which we used to store this call against
...
*/
  async newuac( options ) {
    options.parent = this
    return await call.newuac( options )
  }
  ...
  @param { string } [ options.entity.realm ]
  @param { string } [ options.entity.uri ]
  @return { Promise | Array< Promise > } - returns a promise which resolves to a new call object if a dialog has been confirmed (or failed). If entity is supplied it is an array of promises
  */
  static async newuac( options ) {

    if( undefined !== options.entity ) {
      ...
      for( contact of contactinfo.contacts ) {
        let newoptions = Object.assign( {}, options )
        newoptions.contact = contact
        delete newoptions.entity
        childrenpromises.push( call.newuac( newoptions ) )
      }
    ...
    if( undefined !== options.parent ) {
      newcall.parent = options.parent
      options.parent.children.add( newcall )
    }
