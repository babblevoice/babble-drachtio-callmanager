...
class call {
  ...
  */
  ...
  async _runattendedxfer( req, res, replaces, replacesuri ) {
    ...
    const bothhaveaudio = [ a_1, c_1 ].every( ( call ) => Boolean( call?.channels?.audio ) )
    ...
    */
    ...
    if( bothhaveaudio ) {
      ...
      if( !a_1?.channels?.audio && c_1?.channels?.audio ) c_1.channels.audio.play( c_1._ringsoup )
      if( !c_1?.channels?.audio && a_1?.channels?.audio ) a_1.channels.audio.play( a_1._ringsoup )
    }
