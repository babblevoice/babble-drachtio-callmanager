...
class call {
  ...
  */
  ...
  /**
   * @param { object } preferredchannel - allow to specify a, not necessarily related channel,
   * that can be used to open the new channel's node.
   *
   * @returns { Promise }
   */
  async #choosecodecforanswer( preferredchannel = undefined ) {
    if( this._req.msg && this._req.msg.body ) {
      ...
      channeldef.preferredchannel = preferredchannel
  ...
  */
  async answer( options = {} ) {
    ...
    if ( options.preferredchannel )
      await this.#choosecodecforanswer( options.preferredchannel )
    else
  ...
  */
  ...
  async #openrelatedchannel( channeldef ) {
    ...
    else if ( channeldef.preferredchannel )
      this.channels.audio = await channeldef.preferredchannel.openchannel( channeldef, this._handlechannelevents.bind( this ) )
    else
  ...
  */
  static async newuac( options, callbacks = {} ) {
    ...
    await newcall.#openchannelsfornewuac( { "preferredchannel" : options.preferredchannel } )
