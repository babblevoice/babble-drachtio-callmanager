...
class call {
  ...
  */
  constructor( req, res ) {
    ...
    */
    /**
    For inbound calls - this is doscovered by authentication. For outbound
  ...
  get remote() {
    ...
    switch( this.type ) {
      case "uac": {
        ...
        /* "07766088671" <sip:07766088671@bling.babblevoice.com>;party=calling;screen=yes;privacy=off */
        if( this.entity ) {
          return {
            "name": this.entity.display,
            "uri": this.entity.uri,
            "user": this.entity.username,
            "host": this.entity.realm,
            "privacy": false,
            "type": "calledid"
          }
        }
      ...
      default: {
        ...
        return {
          "name": !parsed.name?"":parsed.name.replace( /['"]+/g, "" ),
          "uri": parsed.uri,
          "user": parseduri.user,
          "host": parseduri.host,
  ...
  */
  ...
  @param { number } [ options.entity.max ] - if included no more than this number of calls for this entity
  ...
  @return { Promise< call | false > } - returns a promise which resolves to a new call object if a dialog has been confirmed (or failed).
  */
  async newuac( options, early ) {
    options.parent = this
    return await call.newuac( options, early )
  }
  ...
  */
  static async newuac( options, callbacks ) {
    ...
    if( undefined === options.contact ) {
      ...
      let othercalls = []
      let origearlycallback = callbacks.early
      callbacks.early = ( c ) => {
        othercalls.push( c )
        if( origearlycallback ) origearlycallback( c )
      }
      ...
      for( let contact of contactinfo.contacts ) {
        ...
        newoptions.contact = contact
        childrenpromises.push( call.newuac( newoptions, callbacks ) )
      }

      for( var i = 0; i < contactinfo.contacts.length; i++ ) {

        let failed = false
        let child = await Promise.race( childrenpromises )
        .catch( ( reason ) => {
          failed = true
        } )
    ...
    if( callbacks && callbacks.confirm ) await callbacks.confirm( newcall )
