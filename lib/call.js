 ...
 */
  ...
  */
  ring() {
    ...
    if ( this.established && "uas" === this.type ) {
      this.play( this.ringsoup )
    }
    else if( !this.ringing && "uas" === this.type ) {
      this.state.ringing = true
      this._res.send( 180, {
        headers: {
          "User-Agent": "project",
  ...
  * @param { object } soup - sound soup as described in projectrtp
  */
  set ringengaged( soup ) {
      this._ringengaged = soup 
  }
  
  /**
   * Return the current ringengaged
   * @returns { object } sound soup
   */
  get ringengaged() {
    if( this._ringengaged ) {
      return this._ringengaged
    }
    return callmanager.options.ringengaged
  }
